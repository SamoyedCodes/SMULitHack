...
def test_unknown_ocr_confidence_cannot_be_high():
    pages = [page(1, [span(1, 0, 'Acme Pte Ltd', source='ocr')])]
    evidence, errors = resolve_citations([cite([f'{DOC}:p1:s0'], 'Acme Pte Ltd')], pages, {DOC})
    assert not errors and evidence[0].source == 'ocr'
    assert evidence_confidence(evidence)[0] == 'medium'


def test_missing_pages_and_context_cannot_be_accepted_with_high_confidence():
    extraction = Extraction(title='Acme', findings=[_parties_draft()])
    review = SupportReview(verdicts=[Verdict(item_id='f1', status='supported', reason='stated')])
    doc = apply_extraction(document(), extraction, review, sample_pages()[:1])
    assert doc.findings[0].confidence == 'low'
    review.verdicts[0].missing_context = ['Schedule 1 is absent']
    doc = apply_extraction(document(), extraction, review, sample_pages())
    assert doc.findings[0].provenance == 'unresolved'
    assert any('Schedule 1 is absent' in i.missing_facts for i in doc.issues)
