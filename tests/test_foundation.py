...
def test_health_no_key_tools_optional_and_empty_portfolio(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert health.capabilities.ingestion and health.capabilities.extraction and health.capabilities.deadlines
        assert not any(v for k, v in health.capabilities.model_dump().items() if k not in {"ingestion", "extraction", "deadlines"})
