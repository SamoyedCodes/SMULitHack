...
import { ReviewQueue, reviewItems } from './Review'
...
function portfolio(over: Partial<Portfolio> = {}): Portfolio {
  return { mode: 'live', as_of: '2026-09-05', horizon_end: '2026-12-04', sme: null, parties: [], documents: [doc('doc-a'), doc('doc-b')], events: [], issues: [issue], conflicts: [conflict, cleared], comparisons: {}, conflict_scan: {...fixture.conflict_scan, state: 'ready'}, coverage: {}, ...over }
}
...
it('counts each current assessment once and excludes stale wrappers and historical results', () => {
  const stale = {...conflict, id: 'old', current: false}
  const wrapper = {...issue, id: 'conflict:conflict-1', kind: 'conflict'}
  const p = portfolio({conflicts: [conflict, stale, cleared], issues: [issue, issue, wrapper, {...wrapper, id: 'conflict:old'}]})
  expect(reviewItems(p).map(item => item.id)).toEqual(['conflict-1', 'issue-1'])
  const html = renderToStaticMarkup(<ReviewQueue portfolio={p} enabled={false} onOpen={() => {}} />)
  expect(html).toContain('2 open')
  expect(html).toContain('stale or refreshing')
