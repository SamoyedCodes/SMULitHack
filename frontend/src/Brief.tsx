...
export function briefHtml(brief: BriefData) {
  ...
  const docs = brief.documents.map(d => escapeHtml(d.title)).join(', ')
  ...
  const sources = brief.evidence.map(e =>
    `<div class="source"><div class="label">${escapeHtml(e.document_id)} · Page ${e.page}${e.clause ? ` · ${escapeHtml(e.clause)}` : ''}${e.source === 'ocr' ? ' · OCR' : ''}</div><blockquote>${escapeHtml(e.quote)}</blockquote></div>`).join('')
...
<p class="meta">${docs} · ${label(brief)} · Provenance: ${escapeHtml(brief.provenance)}${brief.confidence ? ` · Confidence: ${escapeHtml(brief.confidence)}` : ''}<br>Generated ${escapeHtml(brief.generated_at)} · As of ${escapeHtml(brief.as_of)} · ${escapeHtml(brief.mode)} workspace</p>
${brief.coverage_warnings.length ? `<div class="warn">${li(brief.coverage_warnings)}</div>` : ''}
...
export function BriefView({ brief, onBack, onViewSource, viewable }:
  { brief: BriefData; onBack: () => void; onViewSource?: (e: Evidence) => void; viewable?: (documentId: string) => boolean }) {
  return <article className="card conflict-card brief-print-root">
    <div className="brief-actions">
      ...
      <div className="brief-buttons">
        <button className="secondary" onClick={() => window.print()}><Printer size={16} />Print / Save as PDF</button>
        <button className="primary" onClick={() => download(brief)}><Download size={16} />Download brief</button>
      </div>
    </div>
    <div className="section-heading"><h2>{brief.title}</h2><span className="badge">{label(brief)}</span></div>
    <p className="brief-meta">{brief.documents.map(d => d.title).join(', ')} · Generated {brief.generated_at} · As of {brief.as_of} · {brief.mode} workspace</p>
    ...
    <section className="brief-section"><h3>Source excerpts</h3>{brief.evidence.length ? <div className="sources">{brief.evidence.map((e, i) => <div className="source" key={i}><div className="source-label"><Quote size={14} />{brief.documents.find(d => d.id === e.document_id)?.title ?? e.document_id} · Page {e.page}{e.clause ? ` · ${e.clause}` : ''}{e.source === 'ocr' ? ' · OCR' : ''}</div><blockquote>{e.quote}</blockquote>{onViewSource && <button className="contract-link" disabled={viewable ? !viewable(e.document_id) : false} onClick={() => onViewSource(e)}>View source</button>}</div>)}</div> : <p className="brief-meta">No source excerpts are attached to this item.</p>}</section>
...
export function Brief({ id, portfolio, enabled, onBack }: { id: string; portfolio: Portfolio; enabled: boolean; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    void fetchBrief(id, controller.signal)
  ...
  return <>
    <BriefView brief={brief} onBack={onBack} onViewSource={viewSource} viewable={documentId => docLookup.has(documentId)} />
