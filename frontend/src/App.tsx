...
export default function App() {
  ...
  return <div className="app-shell foundation-shell">
    ...
    <div className="main-shell"><header className="topbar"><div>Workspace <ChevronRight size={14} /><strong>{view}</strong></div><span role="status" className={`badge ${connected ? 'green' : 'amber'}`}>{status}</span></header><main>
      ...
      {selectedDocument ? <><ExtractionAction document={selectedDocument} enabled={Boolean(connected && health?.capabilities.extraction)} onRefresh={() => void refresh()} /><div className="contract-detail"><Findings document={selectedDocument} stale={stale || !['complete','needs_review'].includes(selectedDocument.status)} onEvidence={setEvidence} /><SourceViewer backLabel={view === 'Conflicts' ? '← Conflicts' : undefined} key={selectedDocument.id} document={selectedDocument} evidence={evidence} enabled={ingestionEnabled} onClose={() => setSelectedId(null)} /></div></> : <>
      {view === 'Overview' ? <><Readiness health={health} stale={stale} /><section className="card foundation-empty"><FolderOpen size={30} /><h2>{portfolio?.documents.length ? `${portfolio.documents.length} saved document records${stale ? ' · stale' : ''}` : 'No contracts have been analyzed in this build'}</h2><p>{portfolio?.documents.length ? 'Original files stay local. Requested extraction sends page text to OpenRouter or Gemini; text readiness alone is not analysis.' : 'Add contracts to read their pages locally. An empty workspace tells us nothing about obligations in unprocessed documents.'}</p></section>{health && <section className="card capability-card"><h2>Available capabilities{stale ? ' · stale' : ''}</h2><ul>{Object.entries(health.capabilities).map(([name, enabled]) => <li key={name}><span>{name.replaceAll('_', ' ')}</span><span>{enabled ? 'Backend enabled' : 'Not enabled'}</span></li>)}</ul><p>{health.inference_notice}</p></section>}</> : view === 'Contracts' ? <><SmeSelector portfolio={portfolio} disabled={!connected || !health?.capabilities.extraction || smeBusy} onChoose={name => void chooseSme(name)} /><DocumentLibrary documents={portfolio?.documents ?? []} enabled={ingestionEnabled} stale={stale} onOpen={openDocument} onRefresh={() => void refresh()} /><RecentBatch revision={checkedAt ?? ''} /></> : view === 'Conflicts' && portfolio && health?.capabilities.conflicts ? <Conflicts portfolio={portfolio} enabled={connected} stale={stale} focused={focusedConflict} onEvidence={openEvidence} onRefresh={() => void refresh()} onChooseSme={() => setView('Contracts')} /> : <section className="card foundation-empty"><FolderOpen size={30} /><h2>{view} is awaiting its implementation phase</h2><p>{explanation[view]}</p></section>}
      </>}
