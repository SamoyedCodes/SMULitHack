...
export function Readiness({ health, stale }: { health: Health | null; stale: boolean }) {
  const rows = [
    ...
    ['Processing worker', health ? (health.worker.running ? 'Running' : health.worker.enabled ? 'Enabled, stopped' : 'Disabled') : 'Checking', 'The worker reads files locally and processes explicitly requested Gemini extraction jobs.'],
  ]
